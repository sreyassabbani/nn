...
use crate::{
    Float, Sample,
    blueprint::{InitConfig, dense, features_input, relu},
};
...
#[test]
fn blueprint_training_decreases_loss_with_seeded_shuffle() {
    let arch = features_input::<1, _>(dense::<8>().then(relu()).then(dense::<1>()));
